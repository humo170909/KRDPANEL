...
export function initCambiarPass() {
  ...
  document.getElementById("formCambiarPass")?.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      // Limpiar la sesión auth de Supabase inmediatamente tras updateUser.
      // Sin este signOut, el cliente queda en rol 'authenticated' y las queries
      // posteriores evitan las políticas RLS del rol 'anon'.
      await supabaseClient.auth.signOut({ scope: "local" }).catch(() => {});

      if (updateErr) {
        // No exponer el mensaje interno de Supabase (puede contener info del esquema)
        errNueva.textContent = "Error al actualizar la contraseña. Intenta de nuevo.";
        return;
      }
