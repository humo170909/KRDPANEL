...
async function _registrarSalidaColab(callbacks) {
  ...
  const mesId   = getMesActivoAsistencia();
  const registro = await getRegistroColabHoy(mesId);
  if (!registro) { mostrarToast("Primero debes registrar tu entrada.", "error"); await renderColabAsistencia(); return; }
  if (registro.salida) { mostrarToast("Tu salida ya fue registrada hoy.", "error"); await renderColabAsistencia(); return; }
...
async function _adminRegistrarEntrada(form, mesId, callbacks) {
  ...
  const preview = `<strong>Trabajador:</strong> ${sanitize(nom.value.trim())}<br><strong>Hora entrada:</strong> ${ent.value}<br><strong>Estado:</strong> ${estadoEntrada}<br><strong>Fecha:</strong> ${fechaRegistro}<br><strong>Registrado por:</strong> ${sanitize(s?.display || "—")}`;
  if (!(await mostrarGuardar("¿Confirmar registro de entrada?", preview))) return;
...
async function _guardarEdicionAsistencia() {
  ...
  const preview = `<strong>Trabajador:</strong> ${sanitize(campos.nombre)}<br>
    <strong>Fecha:</strong> ${campos.fecha}<br>
    <strong>Entrada:</strong> ${campos.entrada}${campos.salida ? ` &nbsp; <strong>Salida:</strong> ${campos.salida}` : ""}<br>
    <strong>Estado:</strong> ${campos.estado}${campos.justificacion ? `<br><strong>Justificación:</strong> ${sanitize(campos.justificacion)}` : ""}`;

  if (!(await mostrarGuardar("¿Guardar cambios del registro?", preview))) return;
  ...
  document.getElementById("modalEditarAsistOverlay").classList.remove("active");
