...
// ── Utils ──
import { getTodayStr, sanitize } from "./utils/helpers.js";
...
async function _inicializarApp(session) {
  await mostrarApp(session, {
    onAppMostrada: async () => {
      ...
      if (window._ausenciasInterval) clearInterval(window._ausenciasInterval);
      window._ausenciasInterval = setInterval(() => verificarAusencias(), 20 * 60 * 1000);
...
function _initIdleTimer() {
  ...
  const handler = () => {
    ...
    const s = getSession();
    if (!s && document.getElementById("loginScreen").classList.contains("hidden")) {
      clearSession();
      location.reload();
    }
  };
...
function _mostrarPantallaExpulsion(nombreUsuario) {
  ...
  wrap.innerHTML = `
    ...
    ">
      ...
      ">
        ${nombreUsuario
          ? `Hola <strong style="color:var(--text-primary,#e8eeff)">${sanitize(nombreUsuario)}</strong>, tu`
          : "Tu"
        }
...
function _crearModalEstadoEntrada() {
  ...
  div.innerHTML = `
    <div class="modal" style="max-width:380px;text-align:center;">
      ...
      <h3 id="estadoEntradaTitle" style="margin-bottom:10px;font-size:1.15rem;font-weight:700;color:var(--text-primary)"></h3>
      <p id="estadoEntradaDesc" style="color:var(--text-secondary);font-size:0.88rem;margin-bottom:22px;line-height:1.6"></p>
