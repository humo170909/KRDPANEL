...
// Columnas mínimas necesarias para la UI (evitar traer campos innecesarios)
const COLS_OBS = "id, fecha, usuario, mensaje, leida_admin";
...
export async function getObservaciones() {
  ...
  let query = supabaseClient
    ...
    .limit(200);
  // IMPORTANTE: este filtro JavaScript NO es suficiente como control de acceso.
  // Debe estar TAMBIÉN reforzado por una política RLS en Supabase que aplique
  // la misma restricción a nivel de base de datos para el rol anon/authenticated.
  if (s?.role !== "admin") {
