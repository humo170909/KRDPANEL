...
export async function guardarMetaMes(mesId, meta) {
  const s = getSession();
  // Upsert por mes_id: evita el patrón SELECT → INSERT/UPDATE que tiene
  // condición de carrera cuando dos tabs guardan simultáneamente.
  // Requiere restricción UNIQUE(mes_id) en la tabla metas_ventas.
  return supabaseClient
    .from("metas_ventas")
    .upsert(
      { mes_id: mesId, meta, creado_por: s?.display || "—" },
      { onConflict: "mes_id" }
    );
}
