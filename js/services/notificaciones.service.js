...
export function msgTardanza(colaborador, hora) {
  return `⏰ *KRD RRHH — Tardanza*\n👤 ${colaborador}\n🕐 Ingresó a las ${hora}\n📅 ${_fechaHoy()}`;
}

export function msgFalta(colaborador) {
  return `❌ *KRD RRHH — Falta*\n👤 ${colaborador}\n📅 ${_fechaHoy()}\nNo registró asistencia.`;
}
...
export function msgCambioAdmin(accion, usuario) {
  return `🔧 *KRD Admin — Cambio crítico*\n✏️ ${accion}\n👤 Por: ${usuario}\n📅 ${_fechaHoy()}`;
}

export function msgSancion(colaborador, descripcion) {
  return `⚠️ *KRD RRHH — Sanción registrada*\n👤 ${colaborador}\n📋 ${descripcion}\n📅 ${_fechaHoy()}`;
}

export function msgAusenciaDetectada(colaboradores) {
  const lista = colaboradores.map((c) => `• ${c}`).join("\n");
  return `❌ *KRD RRHH — Ausencias detectadas*\n${lista}\n📅 ${_fechaHoy()}`;
}
